...
impl Writer<'_> {
    ...
    pub fn write_posts(&self, posts: &[Post]) -> Result<()> {
        ...
        .try_for_each(|page| {
            make_parent_dir(&mut seen_dirs, &page.file_path)?;
    ...
    pub fn write_static_files(
        ...
        for static_file in static_files {
            make_parent_dir(&mut seen_dirs, &static_file.destination)?;
            std::fs::hard_link(&static_file.source, &static_file.destination)?;
        }
        Ok(())
    }
}
